...
const robot = document.getElementById('roboGif');
...
function tellMe(joke) {
    VoiceRSS.speech({
        key: '9daccdb7733d4502b15cc2c2ed61f2e2', //yaya i know. ii's just a free GET api
        src: joke,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
// Animate Robot
const bounceClasses = ['animate__animated', 'animate__bounce', 'animate__infinite'];

function bounceRobot() {
    bounceClasses.forEach(element => robot.classList.add(element));
}

function unBounceRobot() {
    bounceClasses.forEach(element => robot.classList.remove(element));
}

// Event Listeners
jokeButton.addEventListener('click', getJokes);
audioElement.addEventListener('play', bounceRobot);
audioElement.addEventListener('ended', () => {toggleButton(); unBounceRobot()});
